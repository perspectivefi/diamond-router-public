...
interface IFunctionManager {
    enum FunctionUpgradeAction {
        Add,
        Replace,
        Remove
    }

    struct FunctionUpgrade {
        address implementationAddress;
        FunctionUpgradeAction action;
        bytes4[] functionSelectors;
    }

    event FunctionUpgraded(FunctionUpgrade[] _functionUpgrade, address _init, bytes _calldata);
    ...
    function manageFunctions(
        FunctionUpgrade[] memory _functionUpgrade,
        address _init,
